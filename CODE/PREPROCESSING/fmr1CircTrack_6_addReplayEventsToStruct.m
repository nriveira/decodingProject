...
%
% NOTE:
%   Use fmr1CircTrack_x_replayEvent_SWR_crossCorr to plot the cross
%   correlations between detected population events and SWRs.
% 
% MMD
% 7/2021
% Colgin Lab
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~SLEEPS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                for s = 1:length(group(g).rat(r).day(d).sleep)
                    if ~isempty(group(g).rat(r).day(d).sleep)
                        if ~isempty(group(g).rat(r).day(d).sleep(s).unit)
                            ...
                            badU = []; %initialize
                            for u = 1:length(group(g).rat(r).day(d).sleep(s).unit)
                                if max(group(g).rat(r).day(d).xAllBeginUnitInfo(u).rateMap) >= minFr
