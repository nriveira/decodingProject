    ...
    for r = 1:length(group(g).rat)
        ...
        for d = 1:length(group(g).rat(r).day)
            ...
            for tt = 1:length(tetNums)
                ...
                for s = 2:5 %after experience
                    ...
                    for i = 1:length(group(g).rat(r).day(d).sleep(s).popEv)
                        ...
                        startTm = group(g).rat(r).day(d).sleep(s).popEv(i).tms(1);
                        endTm = group(g).rat(r).day(d).sleep(s).popEv(i).tms(2);
                        slope = group(g).rat(r).day(d).sleep(s).popEv(i).slope;
                        r2 = group(g).rat(r).day(d).sleep(s).popEv(i).r2;
                        ...
                        tmpTms = lfpStruct.ts(startInd:endInd)';
            ...
            for sInd = 1:2
                ...
                end %if there is any data to add
            end %slope ind
...
figtitle = 'ReplayEvent_TFR_inEvent';
if preWhiten == 1
...
for g = 1:2
    ...
    ylabel('Frequency (Hz)')
    ylim(fRange)
    xlabel('Normalized time in replay event')
    xticks([0 1])
    title(group(g).name)
...
figtitle = 'ReplayEvent_TFR_byEvType';
if preWhiten == 1
    figtitle = [figtitle '_preWhiten'];
end %add prewhiten to title
...
figtitle = 'ReplayEvent_powerXfreq';
if preWhiten == 1
    figtitle = [figtitle '_preWhiten'];
end %add prewhiten to title
...
figtitle = 'ReplayEvent_powerXfreq_byEvType';
if preWhiten == 1
    figtitle = [figtitle '_preWhiten'];
end %add prewhiten to title
...
figtitle = 'ReplayEvent_peakRippleFreq';
if preWhiten == 1
    figtitle = [figtitle '_preWhiten'];
end %add prewhiten to title
