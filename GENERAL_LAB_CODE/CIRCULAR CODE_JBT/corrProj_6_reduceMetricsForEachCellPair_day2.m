function cellRegion = corrProj_6_reduceMetricsForEachCellPair_day2(region)
...
for reg = 1%:2
    ...
    for r = 1:length(region(reg).rat)
        ...
        for s = 1:length(region(reg).rat(r).session)
            fprintf('\t\tSession %d\n', s);
            for d = 2 %:numel(region(reg).rat(r).session(s).day)
                fprintf('\t\t\tDay %d\n', d);
                
                if ~isempty(region(reg).rat(r).session(s).day(d).state) %one rat only had 1 cell on one of the days, so there is no data there
                    
                    % # of cell pairs might not be even across all states, since sometimes cells didn't fire during sleep, so figure out the min #
                    numCps = [0 0 0 0];
                    for ss = 1:4
                        if ss <= numel(region(reg).rat(r).session(s).day(d).state) && ~isempty(region(reg).rat(r).session(s).day(d).state(ss))
                            numCps(ss) = length(region(reg).rat(r).session(s).day(d).state(ss).cellPair);
                            if numCps(ss) == 0
                                break
                            end
                        end
                    ...
                    for cp = 1:numCps
                        ...
                        for ss = 1:4
                            ...
                            if badFlag ~= 1 %if the pair was flagged, in a previous state, as a bad pair based on minimum spike count in non-norm spike cross corr
                                ...
                                for bs = 1:4
                                    stXCorrVals = zeros(length(region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).stXCorr{1}{bs}),1);
                                    for b = 1:length(region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).stXCorr)
                                        if ~isempty(region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).stXCorr{b})
                                            stXCorrVals = stXCorrVals + region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).stXCorr{b}{bs}';
                                        end
                                    end
                                    cellRegion(reg).cellPair(cpCntr).day(d).state(ss).stXCorr{bs} = stXCorrVals;
                                    
                                    %check to make sure there was a peak cross-correlation of at least 5 spikes
                                    if bs == 1
                                        maxVal = max(stXCorrVals);
                                        if maxVal < 5
                                            badPairs = [badPairs cpCntr]; %#ok -- if not, mark it as a bad pair, don't continue going through the data for this pair
                                            badFlag = 1;
                                        end
                                    end
                                    ...
                                    if u1TetNum == u2TetNum
                                        if bs == 1
                                            ...
                                            if stXCorrVals(ctrBinInd) > 1.5*avgStXCorr
                                                badPairs = [badPairs cpCntr]; %#ok -- if not, mark it as a bad pair, don't continue going through the data for this pair
                                                badFlag = 1;
                                            end
                                        end
                            ...
                            if badFlag ~= 1 %if the pair was flagged, within this state, as a bad pair based on minimum spike count in non-norm spike cross corr
                                ...
                                for bs = 1:4
                                    normStXCorr = [];
                                    for b = 1:length(region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).normStXCorr)
                                        if ~isempty(region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).normStXCorr{b})
                                            normStXCorr = [normStXCorr; region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).normStXCorr{b}{bs}]; %#ok
                                        end
                                    end
                                    normStXCorr = nanmean(normStXCorr,1);
                                    cellRegion(reg).cellPair(cpCntr).day(d).state(ss).normStXCorr{bs} = normStXCorr;
                                    ...
                                    if bs == 1 %2 ms bin
                                        ...
                                        cellRegion(reg).cellPair(cpCntr).day(d).state(ss).midSum = midSum;
                                    end
                                    clear normStXCorr;
                                end
                                ...
                                if ss == 1 || ss == 4
                                    ...
                                    cellRegion(reg).cellPair(cpCntr).day(d).state(ss).rmCorrCoeffs = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).rmCorrCoeffs;
                                % FOR RATE MAP CROSS CORRELATIONS, GET THE MEDIAN ACROSS BOUTS
                                    cellRegion(reg).cellPair(cpCntr).day(d).state(ss).rmXCorr = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).rmXCorr;
                                    ...
                                    if reg == 1  && ss==1 %MEC
                                        ...
                                        %   2D
                                        cellRegion(reg).cellPair(cpCntr).day(d).state(ss).relSpatPhi2D = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).relSpatPhi;
                                        %   1D
                                        cellRegion(reg).cellPair(cpCntr).day(d).state(ss).relSpatPhiMag = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).spatPhiMag;
                                        
                                        
                                        % FOR GRID SIZE, TGET THE AVERAGE ACROSS BOUTS
                                        cellRegion(reg).cellPair(cpCntr).day(d).state(ss).gridSize = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).gridSize;
                                        
                                        
                                        % GET SPATIAL PERIOD RATIO
                                        spatPerRatio = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).spatPerRatio;
                                        cellRegion(reg).cellPair(cpCntr).day(d).state(ss).spatPerRatio = spatPerRatio; 
                                        
                                        if spatPerRatio < 0.7  %if grid cell sizes are not within 30% of one another, discard this grid cell
                                            badPairs = [badPairs cpCntr]; %#ok
                                        end
    ...
    % get rid of cells pairs that didn't spike enough in the cross-corr
    badPairs = unique(badPairs);
    cellRegion(reg).cellPair(badPairs) = [];
