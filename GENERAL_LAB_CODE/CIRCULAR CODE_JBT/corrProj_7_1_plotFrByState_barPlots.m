function region = corrProj_7_1_plotFrByState_barPlots(region)
...
for reg = 1:2
    ...
    statReg(reg).frXUnits = xRatUnitFrInfo; %#ok
    statReg(reg).ANOVA_stats = [A.df A.F A.p pEta2]; %#ok
    statReg(reg).cellCount = size(xRatUnitFrInfo,1); %#ok
    statReg(reg).avgsAndErr = [mean(xRatUnitFrInfo,1); semfunct(xRatUnitFrInfo,1)]; %#ok
...
RUNCOLOR{1} = [0.0 0.5 0.0];
RUNCOLOR{2} = [1.0 0.54688 0.0];
RUNCOLOR{3} = [0.29297 0.0 0.50781];
for reg = 1:2
    subplot(1,2,reg); 
    hold on; 
...
%         frs = statReg(reg).frXUnits(:,s);
%         xx = (s-1) + ones(size(frs)) + 0.3*rand(size(frs))-0.15;
%         scatter(xx,frs,'filled', 'markerfacecolor', RUNCOLOR{s});
%         set(bg, 'FaceColor', rgb(stateCols{s}))
    end
    boxplot(statReg(reg).frXUnits)
