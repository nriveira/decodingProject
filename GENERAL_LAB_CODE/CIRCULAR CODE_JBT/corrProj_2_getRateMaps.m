...
LTBinEdge_x = 0:gridBinSize:174-gridBinSize;
LTBinEdge_y = 0:gridBinSize:98-gridBinSize;
                        ...
                        if reg == 1 %MEC
                            ...
                            if(tNum==2)
                                ...
                                for x = 1:floor(100/gridBinSize) %100 cm/binSize = #Bins in x dim
                                 ...
                                 numBinSamps = zeros(floor(174/gridBinSize), floor(98/gridBinSize));
                                numBinSpks = zeros(floor(174/gridBinSize), floor(98/gridBinSize), length(region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit));

                                % GET AMOUNT OF TIME MOVING IN EACH GRID BIN
                                for x = 1:floor(174/gridBinSize) %100 cm/binSize = #Bins in x dim
                                    xBinLims = [(x-1)*gridBinSize x*gridBinSize];
                                    for y = 1:floor(98/gridBinSize) %100 cm/binSize = #Bins in x dim
                                        yBinLims = [(y-1)*gridBinSize y*gridBinSize];
                            ...
                            for u = 1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit)
                                if(region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit(u).type~=1)
                                    continue;
                                end
                                    ...
                                    for st = 1:length(spkTms)
                                        ...
                                        if velFilt(stInd) == 1 %if rat was moving fast enough when spike occurred
                                            if tNum==2
                                                ...
                                                spkXPos = coords(stInd,2);
                                                xBin = find(LTBinEdge_x<=spkXPos,1, 'Last');
                                                spkYPos = coords(stInd,3);
                                                yBin = find(LTBinEdge_y<=spkYPos,1, 'Last');
                                                numBinSpks(xBin,yBin,u) = numBinSpks(xBin,yBin,u) + 1;
                                            end
                                        end
                                    end
