...
%     for r = 4
    ...
    %                 end
                    if(~isdir(region(reg).rat(r).session(s).day(d).task(tNum).name))
                        continue;
                    end
                    ...
                    for u1 = 1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit)
                        u1ID = [region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit(u1).ID, region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit(u1).type];
                        for u2 = u1+1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit)
                            u2ID = [region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit(u2).ID, region(reg).rat(r).session(s).day(d).task(tNum).bout(1).unit(u2).type];
                    ...
                    for b = 1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout)
                        ...
                        if ~isempty(runEpochBnds)
                            ...
                            for u1 = 1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit)
                                u1ID = [region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit(u1).ID, region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit(u1).type];
                                ...
                                for u2 = u1+1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit)
                                    ...
                                    u2ID = [region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit(u2).ID, region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit(u2).type]; %get unit 2 ID
                                    ...
                                    for bs = 1:length(spkBinDur)
                                        ...
                                        if bs == 1
                                            ...
                                            if(~isempty(sum(cpCc{bs}(tmpInds))))
                                    ...
                                    cpInd = find(cellPairList(1,1,:)==u1ID(1) & cellPairList(1,2,:)==u1ID(2) & cellPairList(1,3,:)==u1ID(3) & cellPairList(2,1,:)==u2ID(1) & cellPairList(2,2,:)==u2ID(2) & cellPairList(2,3,:)==u2ID(3));
                                    if(~isempty(cpInd))

                                        %store all the variables we might need
                                        if(tNum==2)
                                            region(reg).rat(r).session(s).day(d).state(1).cellPair(cpInd).midSum(:,b) = midSum;
                                            region(reg).rat(r).session(s).day(d).state(1).cellPair(cpInd).stXCorr{b} = cpCc;
                                            region(reg).rat(r).session(s).day(d).state(1).cellPair(cpInd).normStXCorr{b} = nCpCc;
                                        elseif(tNum==1)
                                            try
                                            region(reg).rat(r).session(s).day(d).state(4).cellPair(cpInd).midSum(:,b) = midSum;
                                            region(reg).rat(r).session(s).day(d).state(4).cellPair(cpInd).stXCorr{b} = cpCc;
                                            region(reg).rat(r).session(s).day(d).state(4).cellPair(cpInd).normStXCorr{b} = nCpCc;
                                            catch
                                                keyboard
                                            end
                                        end
                                    end
                                 %if unit2 is a grid cell/pyram
                            end %unit2
                                    ...
                                    %                                     fprintf('\t\t\t\t\t\t\tUnit %d (Tet#%d, Unit#%d)\n', u1, u1ID(1), u1ID(2));
                                        ...
                                        for u2 = u1+1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout(b).unit)
                                            ...
                                            %                                             fprintf('\t\t\t\t\t\t\t\tPaired with Unit %d (Tet#%d, Unit#%d)\n', u2, u2ID(1), u2ID(2));
                                                ...
                                                if(~isempty(cpInd))
                                                    try
                                                        region(reg).rat(r).session(s).day(d).state(ss+1).cellPair(cpInd).midSum(:,b) = midSum;
                                                        region(reg).rat(r).session(s).day(d).state(ss+1).cellPair(cpInd).stXCorr{b} = cpCc;
                                                        region(reg).rat(r).session(s).day(d).state(ss+1).cellPair(cpInd).normStXCorr{b} = nCpCc;
                                                    catch
                                                        keyboard
                                                    end
                                                end
