                        ...
                        for ss = 1:4
                            ...
                            if badFlag ~= 1 %if the pair was flagged, in a previous state, as a bad pair based on minimum spike count in non-norm spike cross corr
                                ...
                                if ss == 1
                                    ...
                                    unitInfo = region(reg).rat(r).session(s).day(d).state(ss).cellPair(cp).unitIDs;
                            ...
                            if badFlag ~= 1 %if the pair was flagged, within this state, as a bad pair based on minimum spike count in non-norm spike cross corr
                                ...
                                if ss == 1
                                    ...
                                    cellRegion(reg).cellPair(cpCntr).state(ss).rmCorrCoeffs = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).rmCorrCoeffs;
                                    
                                    if reg == 1 %MEC
                                        
                                        % FOR RATE MAP CROSS CORRELATIONS, GET THE MEDIAN ACROSS BOUTS
                                        cellRegion(reg).cellPair(cpCntr).state(ss).rmXCorr = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).rmXCorr;
                                        
                                        
                                        % FOR RELATIVE SPATIAL PHASE, GET THE MEDIAN ACROSS BOUTS
                                        %   2D
                                        cellRegion(reg).cellPair(cpCntr).state(ss).relSpatPhi2D = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).relSpatPhi;
                                        %   1D
                                        cellRegion(reg).cellPair(cpCntr).state(ss).relSpatPhiMag = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).spatPhiMag;
                                        
                                        
                                        % FOR GRID SIZE, TGET THE AVERAGE ACROSS BOUTS
                                        cellRegion(reg).cellPair(cpCntr).state(ss).gridSize = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).gridSize;
                                        
                                        
                                        % GET SPATIAL PERIOD RATIO
                                        spatPerRatio = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).spatPerRatio;
                                        ...
                                        % FOR RELATIVE ANGULAR DISTANCE, GETTHE MEDIAN ACROSS BOUTS
                                        cellRegion(reg).cellPair(cpCntr).state(ss).relSpatDist = region(reg).rat(r).session(s).day(d).state(1).cellPair(cp).relSpatDist;
