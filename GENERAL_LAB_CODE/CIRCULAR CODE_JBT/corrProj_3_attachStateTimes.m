...
for reg = 1 %:2
    % for reg = 1
    fprintf('%s\n', region(reg).name)
    cd([dataDir,'\',region(reg).name])
    for r = 1:length(region(reg).rat)
        %     for r = 6
        fprintf('\t%s\n', region(reg).rat(r).name);
        cd([dataDir,'\',region(reg).name,'\',region(reg).rat(r).name])
        for s = 1:length(region(reg).rat(r).session)
            ...
            for d = 1:length(region(reg).rat(r).session(s).day)
                ...
                for(tNum=1:2) %MEC: 1:Linear Track, 2:Open Field
                    ...
                    for b = 1:length(region(reg).rat(r).session(s).day(d).task(tNum).bout)
                        ...
                        deltaPow = traces2TFR(eegStruct.data,2:5,eegStruct.sf,5);
                        deltaPow = mean(deltaPow,1); %average across the frequency range

                        %calculate theta power
                        thetaPow = traces2TFR(eegStruct.data,6:10,eegStruct.sf,5);
                        thetaPow = mean(thetaPow,1);  %average across the frequency range

                        %calculate TDR
                        tdr = thetaPow./deltaPow;

                        [mwTdr, mwInds] = mw_avg(tdr, tdrMwSize*eegStruct.sf, tdrMwStep*eegStruct.sf,0);
                        halfWin = (tdrMwSize*eegStruct.sf)/2;
