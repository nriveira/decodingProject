...
%  This code is copied exactly from that provided by Sean, which was provided
%  by Laura, which was handed down from Mosers.
%  The only thing I'm adding (JT) is comments at the top so you know what it's doing.
%
% INPUT:
...
[numBins_x, numBins_y] = size(map1);

if(numBins_x > numBins_y)
    map1=[map1,nan(numBins_x, numBins_x-numBins_y)];
    map2=[map2,nan(numBins_x, numBins_x-numBins_y)];
    numBins=numBins_x;
elseif(numBins_y > numBins_x)
    map1=[map1;nan(numBins_y-numBins_x, numBins_y)];
    map2=[map2;nan(numBins_y-numBins_x, numBins_y)];
    numBins=numBins_y;
else
    numBins=numBins_x;
end
