function RunBayesianDecoder_ripple_BeforeTest(Path,CSClist,Outdir,cellList,spktime_fd,trackdata,plotOnly)

outdir = strcat(Outdir,'\BayesianDecoding_ripple_BeforeTest');
if ~isdir(outdir)
    mkdir(outdir)
end
if exist(strcat(outdir,'\BayesianDecodingResult.mat'),'file')==2 && plotOnly
    ...
    for ii = 1:length(Path)
        ...
        BayesianDecodingResult.(RatID).(DateID).slope = slope; % in rad/sec
...
NUniqueCell_thr = 0;
NTrial_all = [];
r2_correctVSerror_sampTest=[];
r2_correctVSerror_postTest=[];
...
for rr = 1:length(ratID)
    ...
    for dd = 1:length(dateID)
        ...
        %Obtain mean r2 values between correct and error trials during
        %sample/test phase
        if length(correct_test) >= 3 && max(NTrial)-length(correct_test) >= 3 % at least 3 correct and error trials
           correct_ind = ismember(NTrial(include1 & include2),correct_test);
           r2 = BayesianDecodingResult.(ratID{rr}).(dateID{dd}).r_squared(include1 & include2);           
           r2_correctVSerror_sampTest = cat(1,r2_correctVSerror_sampTest,[mean(r2(correct_ind)) mean(r2(~correct_ind))]);
        end
        
        %Obtain mean r2 values between correct and error trials during
        %post-test phase
        if length(correct_posttest) >= 2 && max(NTrial_post)-length(correct_posttest) >= 2 % at least 2 correct and error trials
           correct_ind = ismember(NTrial_post(include3),correct_posttest);
           r2 = BayesianDecodingResult.(ratID{rr}).(dateID{dd}).r_squared(include3);           
           r2_correctVSerror_postTest = cat(1,r2_correctVSerror_postTest,[mean(r2(correct_ind)) mean(r2(~correct_ind))]);
        end
    end
end
posbinsize = BayesianDecodingResult.(ratID{rr}).(dateID{dd}).param.posbinsize;
...
for r1 = 1:length(Replay_alignment)
    for p1 = 1:length(Position_alignment)
        ...
        h = figure;
        set(h,'OuterPosition',[13.8,84.2,1511.2,776.8])
        ha = zeros(length(sessions),1);
        ha3 = zeros(length(sessions),1);
        for ss = 1:length(sessions)
            in = strcmp(sessionType,sessions{ss}) & r_squared >= r2_thr & duration >= dur_thr & emptyBin_percent <= emptyBin_thr;
            pxn = pxn_aligned(in);
            ...
            ha(ss) = subplot(3,length(sessions),ss);
            ...
            subplot(3,length(sessions),ss+length(sessions))
            ...
            % Plot distribution of slope
            s = slope(in)*180/pi; % convert to deg/sec
            ha3(ss) = subplot(3,length(sessions),ss+2*length(sessions));
            histogram(s,20);
            axis square
            colorbar            
            if ss == 1
                xlabel('Slope(deg/sec)')
                ylabel('count')
            end            
        end
        colormap hot
        ...
        saveas(h,strcat(outdir,'\',fname1,'_',fname2),'epsc')
        saveas(h,strcat(outdir,'\',fname1,'_',fname2),'png')
        close(h)       
...
%% Plot r2 distributions between correct and error trials (combined early and late learning)
h = figure;
set(h,'OuterPosition',[374.6,141,920.8,560.8])
ha = zeros(2,1);
for rr = 1:2
    if rr == 1
        in = strcmp(sessionType,'InitialFour')|strcmp(sessionType,'LastFour');
        titlestr='Sample/Test';
    else
        in = strcmp(sessionType,'PostTest');
        titlestr='Post-test';
    end
    ...
    ha(rr) = subplot(1,2,rr); hold on
    ha1 = shadedErrorBar(axisValue1,Wdis_correct,CI_correct,'r-');
    ha2 = shadedErrorBar(axisValue2,Wdis_incorrect,CI_incorrect,'k-');
    ylabel('Proportion')
    xlabel('r^2')
    legend([ha1.mainLine; ha2.mainLine],'String',...
        {strcat('Correct(n=',num2str(sum(Correct_test & in)),')'); strcat('Error (n=',num2str(sum(~Correct_test & in)),')')})
    axis square
    title(titlestr)
end
linkaxes(ha,'y')
saveas(h,strcat(outdir,'\R2distCorrectVSError'),'epsc')
saveas(h,strcat(outdir,'\R2distCorrectVSError'),'png')
close(h)


%% Plot mean r2 values between correct and error trials
h = figure;
set(h,'OuterPosition',[481,334.6,850.4,508])
ha1 = subplot(1,2,1);
plot(r2_correctVSerror_sampTest','-o','Color',[.8 .8 .8])
CI = bootci(5000,@mean,r2_correctVSerror_sampTest);
CI = [mean(r2_correctVSerror_sampTest)-CI(1,:); CI(2,:)-mean(r2_correctVSerror_sampTest)];
hold on; errorbar(1:size(CI,2),mean(r2_correctVSerror_sampTest),CI(1,:),CI(2,:),'k-')
xlim([.5 size(CI,2)+.5])
axis square
set(gca,'XTick',1:size(CI,2),'XTickLabel',{'Correct', 'Error'})
ylabel('Mean r2')
title('Sample/Test')

ha2 = subplot(1,2,2);
plot(r2_correctVSerror_postTest','-o','Color',[.8 .8 .8])
CI = bootci(5000,@mean,r2_correctVSerror_postTest);
CI = [mean(r2_correctVSerror_postTest)-CI(1,:); CI(2,:)-mean(r2_correctVSerror_postTest)];
hold on; errorbar(1:size(CI,2),mean(r2_correctVSerror_postTest),CI(1,:),CI(2,:),'k-')
xlim([.5 size(CI,2)+.5])
axis square
set(gca,'XTick',1:size(CI,2),'XTickLabel',{'Correct', 'Error'})
title('Post-test')
...
linkaxes([ha1 ha2],'y')
saveas(h,strcat(outdir,'\R2pairCorrectVSError'),'epsc')
saveas(h,strcat(outdir,'\R2pairCorrectVSError'),'png')
close(h)
...
for ii = 1:length(RippleOnsetIndex)
    ...
    if sum(~isnan(BeforeTest(InRange))) == length(InRange)...
        ...
        if full(sum(sum(SpikeMatrix(~rmv,InRange)))) > 0
            ...
            if  sum(~isnan(BeforeTest(InRange))) == length(InRange)
                if mode(BeforeTest(InRange))<=4
                   sessionType{ii} = 'InitialFour';
                   ntrial = mode(BeforeTest(InRange));
                ...
                elseif mode(BeforeTest(InRange))>4
